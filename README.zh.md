...
| 项 | 值 |
|---|---|
| DSH 兼容性 | `@deepseek-ai/dsh` `0.1.7-rc.2`（已确认 `session-telemetry/record` 桥接事件仍存在；无 dsh 服务依赖） |
| 运行时依赖 | 仅 `@deepseek-ai/cordis` `4.0.4` |
| 最后验证 | 基于 `@deepseek-ai/cordis` `4.0.4` / `@deepseek-ai/dsh-settings` `0.1.7-rc.2` 的单元测试 |
| Node | `^22.19 \|\| >=24` |
...
> dsh `0.1.5` 移除了 `installSettingsSection`，`0.1.7-rc.2` 仍未恢复，因此在该等版本上
> 插件会探测此 API 并优雅地空操作——配置仍由文件/环境变量驱动。若未来 dsh 恢复该 API，
> 集成会自动重新生效。
>
> 参考用的 Web 设置**卡片**（客户端部分）停在 `wip/settings-ui` 分支。它早于 dsh 官方的
> client-plugin 机制（`dsh.client` 声明 + `/plugins` 提供 bundle + `ctx.slots.register`，
> 0.1.7 线已发布），重启时需按该契约重做；未随 `main` 发布。
